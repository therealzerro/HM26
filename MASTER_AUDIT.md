...
**GATE — assembled 20260729 with the full package: 21.43s.** Seal stinger resolved, gold ribbon start/mid/end of body verified on frames, lattice tagged endcard resolved, endcard 100% crop clean. **Republished on explicit operator order** (2026-07-30 19:39Z — "push the repaired version live"): storage files overwritten, row upsert reset `status` to `ready` and `posted_at` to null. The prior posted stamp (10:22Z) was against the pre-MKT-31 cut, so the reset reflects reality; the assisted-handoff flow re-flips it on the next post. The MKT-27 no-republish precedent is about not wiping stamps *unasked* — this one was asked. Carry-over noted: the ribbon overlaps the first receipt row's match line mid-scroll — pre-existing stamp geometry (y≈470, pinned by spec), not introduced here; candidate for MKT-25's restaging if it bothers the operator.
