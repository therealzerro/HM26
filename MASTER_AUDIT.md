...
- **BRAND-05 (open):** BRAND-04-banned match-status vocab regressed on consumer surfaces — verified: `app/(tabs)/explore.tsx:510` (`⭐ EXACT`/`🎯 PARTIAL` badges), `app/(tabs)/results.tsx:767` (`Exact/Partial` badge) and `:778` (native Share message carrying "Exact/Partial match" off-device — highest leak risk); agent-reported: `book.tsx:528` `HIT` count label, `_layout.tsx:45` a11y "new hits", `learn.tsx:13` 🎰 emoji, `index.tsx:929` hardcoded "verified 72.4% match rate" (measured 2026-05-27, inside the BUG-162 inflation era — provenance problem). `check:brand-voice` is phrase-level and passes all of these; scanner needs bare-word EXACT/PARTIAL/HIT rules. Consumer surfaces — fixes require explicit operator go-ahead.
- **BRAND-06 / SEC-adjacent (open, verified):** ZK30 tab is un-gated on the public tab bar (`app/(tabs)/_layout.tsx:167-173`; no role check in `zk30.tsx`) — operator instrumentation + "HITS" labels visible to free users; zk30 files absent from brand-scan scope. Proposed: `href: null` unless admin.
- **BUG-164 (open, verified):** Home "matches today" count — BUG-141 dedupe key uses `matched_state` but the query select list omits it (`app/(tabs)/index.tsx:444,459`), so the key always gets `''` and multi-state matches collapse to one, contradicting the HitCard list below. One-line fix (add `matched_state` to select); consumer surface, awaits ask. tsc already flags it (TS2339).
- **BUG-165 (open, agent-reported):** Intelligence Refresh/Retry pass the tap event into `load(force?: boolean)` (`intelligence.tsx:586,921`) — event truthy ⇒ every tap bypasses the stale-cache guard and re-pages the full DI table. Fix: `() => load(true)` / `() => load()`.
- **OBS-PULL-HITDET (open, agent-reported):** consumer pull-to-refresh on Slates runs `runHitDetectionAllScopes` for today AND yesterday per gesture (`explore.tsx:217-230`) — two service-role edge invocations from any anon client; amplification + latency. Proposed: snapshot refresh only.
- **SOCIAL-10 (FIXED 2026-07-23):** group/cross-lane share actions (`shareAllToFacebook`/`prepAndOpen`/`copyCaption`) did NOT gate on `lint.ok` — page lane hard-gated (`PublishView.tsx:431`) but group lanes only rendered warnings. Fix: shared `lintBlocked()` guard inside all three callbacks + disabled/opacity state on Share All / Prep & Open / Copy Caption, mirroring the page-lane hard block (no override path — group lanes never had one). Shipped with SOCIAL-11. Residual (still open, minor): Two-Question ack renders only in the API lane though the brief mandates it for cross-posts; AI group captions get no server-side lint (report Part 1 §4.2–.3).
- **Stale operator string (verified-class, minor):** `DashboardView.tsx:756` claims a nightly 23:30 ET cron — all pg_cron removed under OPS-01.
