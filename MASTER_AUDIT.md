...
1. 7/29 verify row is `ready` / `posted_at` null (reset by republish) — re-flips on the next assisted post. Telegram needs the PRO draft (live caption is the free one; the tier-4 gate refuses it — working as designed).
...
**✅ [[MKT-28]] FULLY CLOSED 2026-07-30 (later) — the delivered neutral vocabulary is IN.** Canonical band scale by rank: ON FIRE/BLAZING/HOT/WARM/COOL → PEAK/HIGH/MID/LOW/BASE BAND; BLAZING SIGNAL → PEAK SIGNAL; STRONG SIGNAL / MODERATE / OVERDUE / LOW keep (already neutral — and LOW may never get a map entry: WARM→'LOW BAND' and the matcher is substring-based). Flame, ice and the degree sign DROP (the ° was caught live on the grid chip: `PEAK BAND 99°` → `PEAK BAND 99`). All 11 terms pass the real tier-1 lint; provisional set replaced outright. 11-row table under the full entry below. **✅ [[MKT-16]] Phase 1+3 SHIPPED the same day** — `allday_public` registered at all five points + the kind CHECK (landed FIRST, operator ruling), assembled, published, Q1/Q2 gate record stored; the four public platforms are now UNBLOCKABLE but stay disabled pending explicit per-platform go-ahead. Full entry at MKT-16. Earlier same day: **✅ [[MKT-28]] ENGINEERING DONE 2026-07-30 — two copy decisions owed back.** 28b: `PickDetailModal` migrated off the surviving pre-DESIGN-02 90/75/60 ramp, which disagreed with its own label thresholds (energy 70 = 65-band label in a 60-band colour). 28a: the `HOT SIGNAL` override centralised as `heatLabelDetailed()` — nothing on screen changed, but reconciling it later is now one edit. 28c: the redaction over-mask fixed structurally after PROBING the DOM, with the matching carve-out added to the assert (without it the surviving `100` aborts the render). **Still owed, and deliberately not decided unilaterally:** whether `HOT SIGNAL` or `HOT` wins, and whether Heat Check keeps its own 85/70/50 — both are copy, both ride with the public-relabel vocabulary, which must cover all five ladders at once. Full entry below.
...
**⚠ TWO DEFECTS CAUGHT BY PREFLIGHT, both real and both this class.** (1) The verify assembler never passed a date to `probeStinger`, so `stingerCandidates` built an empty rotation and fell through to the unversioned file — while `reel:check` reported *"verify stinger selection → imprint"*. **Preflight describing a beat the assembler will not use is exactly the failure the "check the RESOLVED file, not the incumbent" rule exists to prevent.** Verify now rotates like every other kind; its INTRO stays date-less because that one IS a fixed kind by MKT-17 ruling. (2) The pt2 master was first named `*_pt2_master_10s.mp4` — contains `_pt` but does not match `_pt<N>.mp4`, so it was invisible to both the joiner and the orphan scan. The guard was right to FAIL it. Also fixed: `check-reel-assets` asserted a flat 9.2s verify carrier need, true only while the body was 6.3s — it cannot know the need at preflight time and now reports the coverage picture instead of trivially passing.

**⚠ THE PUBLISHED 7/28 VERIFY REEL IS THE OLD CUT.** The local file was rebuilt to 24.43s for verification; the row for 2026-07-28 is still `posted` against the 14.43s version. **Deliberately not republished** — that would wipe a real posted stamp to replace a reel that has already gone out. The first live reel carrying the holds + stinger + pt2 will be whatever the next Daily Workflow run produces. There is also **no verify reel for 7/29** (the 7/29 run built 7/28's, by design — reel_date is the content date).

**Superseded, kept for the trail — CARRIER GAP AS MEASURED BEFORE PT2: 6.49s.** On a 13.60s body the audio ends at 15.24s of a 21.73s reel. `apad` now keeps the audio STREAM as long as the container (it was ~6.5s short — legal mp4, but a platform transcoder is entitled to mishandle it). **The gap is independent of the stinger** (`carrierNeed` derives from `uiDur`, not `openDur`). A `verif_carrier_pt2` of **~7.0s** closes it on every day including the 14.0s worst case; until one exists the silence under the closing holds is deliberate.
...
### MKT-40 — verify_public 🔍 PHASE 0 REPORTED · BUILD GATED (2026-07-31)

**ID verified free** (`grep MKT-40` → audit 0 / code 0; reserved by this report for the build). Operator ruling: verify_public comes AHEAD of the Wave-2 ad lane. Full report in `VERIFY_PUBLIC_PHASE0_2026-07-31.txt`; delivered copy staged verbatim at `docs/verify_public_copy_delivery_2026-07-31.md` (21 items through the real tier-1 lint, **0 trips**). Confirmed a PARALLEL capture build — MKT-30's `--redact --relabel` covers neither /track-record nor the board compositor. Headlines: **(1)** the 30d rollup renders 154/124 three-digit Q1-blocked totals; 7-day is 35/8/27 all-two-digit but only a clamp makes that structural — the structurally-safe shapes drop unbounded totals (DAYS ≤31 and JURISDICTIONS ≤51 are safe by construction). **(2)** complete match-string inventory delivered for both segments; proposed EXACT ORDER / ANY ORDER relabels all lint clean; session words and state codes on ledger rows need drop/suppression; a11y labels are DOM text and must be swept. **(3)** with both digit columns masked the gold hierarchy survives as color+label but the eye-checkable posted==drawn proof collapses — accept / mirrored-placeholder cue (truthful but designed) / partial reveal barred. **(4) FREE WIN CONFIRMED: the whisper env HEALED and the incumbent carrier pair transcribes Q2-clean as spoken** ("Mornin' partner, yesterday's receipts are in, checked against the official draw…" / "That one's on the record. Signals first. Receipts after.") — verify_public shares them at zero Omni cost; the build is cheap, not blocked. **BONUS: the parked part-1 candidate transcribes exactly to script** (last word 9.42s, forbidden sweep clean, A.1 sequencing satisfied by MKT-35) — its content gate PASSES; voice-family + operator swap-or-reject remain, flagged not acted. **(5)** registration inventory enumerated (migration-first, five points, Kind union same-change, KIND_UI, YT per-kind set, `_public`-suffix gates inherit); two config decisions flagged: seal inheritance, chip accent. Headline-dominant stinger layout RULED extended (delegated by the copy delivery). **(6)** cadence facts reported both ways — operator decides. **(7)** cost confirmed ≈ +16-19MB, +4-7min, generation $0. Gate honored: nothing built. Cross-ref [[MKT-16]] (registration precedent), [[MKT-30]] (capture pattern), [[MKT-35]] (board treatment), [[MKT-37]] (transform mechanism), [[MKT-39]] (cost baseline).
...
**ADDENDUM — operator ruled same day: 1:1 CUT RETIRED.** Executed 2026-07-31: all 1:1 objects deleted from the bucket and every `video_1x1_path` nulled (31 references — 8 live objects + stale paths on archived rows); 9 local `_1x1.mp4` files removed (89MB); the 1:1 encode dropped from BOTH assemblers (8 fewer encodes/run) with the resurrect recipe commented at the removal site. No other code changed — publish already existsSync-guards the upload, retention filters null paths, the ReelsView button hides on null, and no registered platform declares a 1:1 asset (platform apps letterbox 9:16 themselves). Bucket steady state 214 → ~135MB. ⚠ The 1:1 centre-crop BAND (y 420-1500) in the delivery spec stays BINDING — it anchors the chip zone and composition clearances; retiring the output does not relax the band, and a future 1:1 need is a code resurrect, not a fleet respec. Filtered tsc 0.
...
**P0 — WHAT WAS POSTED (operator YouTube uploads 7/30 + 7/31).** `marketing_reels` shows `allday_public` handed to the **cross-post** lane both days (7/30 16:46Z, 7/31 10:55Z) — the lane built for uncontrolled surfaces — while every free-tier row went to its normal group target. The DB records handoff targets, not platforms, so this is strong evidence but not proof that the YouTube uploads were the public cuts; **operator confirmation still worthwhile**. Not an incident either way (YouTube carries no probation history). **Guard shipped at the real seam** (the order said reel:check, but preflight cannot see posting targets — the handoff UI can): ReelsView now WARNs, non-blocking, when (a) a tier-1 platform row carries a non-`_public` reel kind, and (b) the cross target is selected on a non-`_public` kind — both say the free/pro cut carries real digits and cannot honestly clear Q1, and point at allday_public.

**P1 — YOUTUBE SHORTS SET, registered as a per-platform TRANSFORM under MKT-24's mechanism** (constants/socialPlatforms.ts): 8 titles (<60 chars) + 4 descriptions rotating INDEPENDENTLY (description 4-cycle advances one extra step per title-cycle wrap → all 32 pairings over 32 days), tags ruling recorded in the registry (`#Shorts #DataAnalysis #PatternAnalysis`, description only, never the title; no gambling-adjacent tags — revisit only with data). `{free_group_url}` wired to the app_config group URL through the handoff row; rotation keyed to the reel's CONTENT date (caption-engine dayOfYear), never the render clock. `platformCaption` gained an optional ctx param — every existing call site unchanged; a future platform (TikTok/X) inherits the mechanism. Reddit still needs its own copy when enabled (90/10 rule vs announcement register — recorded).

**P2 — TEMPLATES 8→12 per kind** in the caption engine, delivered copy wired to the engine's real substitution machinery: `{scope}` → factory label; verify-pro `{n}/{n_states}/{n_straight}/{n_30day}` → ProCtx fields with **straight clauses conditional on `straightJx.length`** ("including 0 STRAIGHT" can never render); verify-free scale words → QualCtx phrases (no line can overclaim on a 1-match day). **Delivered verify-free #12 is gated STRUCTURALLY as ordered**: the straight claim renders only via `straightPhrase`, which `makeQualCtx` emits only when the verified record contains a straight — the rotation cannot produce a false straight claim on the verification reel. **⚠ ONE LINE NOT REGISTERED at first delivery**: verify-free #9 ("Grade day: {date} closed with…") duplicated shipped template 5 verbatim once its scale words went through QualCtx; a replacement was requested rather than silently rewritten. **RESOLVED same day** — the replacement ("Every signal on yesterday's board went up before the draw and got checked after…") arrived, was wired through QualCtx (renders truthfully from "A verified MATCH" up to "A stack of verified MATCHES from coast to coast"), linted clean, and closes verify-free at 12.

**P3 — SPACING RE-VERIFIED AT 12, HOLDS with no re-spacing**: pro trio residues mod 12 = 5/7/11 (pairwise distinct), free sessions 17→5 / 22→10 (distinct); one new harmless cross-family coincidence (midday_free ≡ allday_pro ≡ 5) — same class as the documented evening_pro/allday_free collision at 8. **LINT: zero trips.** All 196 unique rendered captions (every kind × all rotation indices × straight/box-only/one-straight receipt variants) pass the real `lintCaption` at their tiers; all 8 YT titles + all 32 day-rotations of the descriptions pass at tier 1. Filtered tsc 0. Cross-ref [[MKT-24]] (transform mechanism), [[MKT-13]]/[[MKT-26]] (session kinds), [[MKT-16]] (public kind), SOCIAL-13.

---

### MKT-36 — anchor_intro_public replacement (2026-07-31) ✅ ACCEPTED & SERVING (after corruption recovery)

**ID verified free** (grep → 0 before the P0 park entry, stamped by this order). Serving-asset swap, comparative acceptance — "at least as good as what it replaced", not "passes spec".

**P0 — THE DELIVERY PATH ATE TWO MORE BINARIES (running total 6).** The 7/31 drop arrived as upload `8bbd22c` then THREE GitHub-web renames; both the delivery AND the shelved incumbent (`anchor_intro_public_old.mp4`) landed as 2-byte moov-less stubs — **the public lane's only intro was dead on disk** (no rotation fallback exists for allday_public by design). Recovery before any measurement: incumbent restored from `c0bcb42` and serving; rollback parked as `*_incumbent_backup.mp4`; delivery bytes recovered intact from the upload commit (2,773,566 bytes) and parked as candidate; stub removed; every file probed after every move.

**P1 — TRIM + COMPARATIVE GATE: the candidate WINS.** Trim rule applied to the footage: smoke fills the frame ~5.8s (bezel remnants at the left edge through 5.7), the anchor GHOSTS BACK through the smoke from ~6.6s (this master's own version of the pull-back lesson — the out cannot slide later), so OUT 6.4 / IN 0.4, 6.000s exactly, opening on the ignition flare (audio hot from frame 1 — the only silence anywhere is a musical rest 2.22–2.98s). Side-by-side: onset −20.2dB mean/−10.4 max vs incumbent −37.9/−19.6 (hotter open on the one surface facing cold viewers); bed 5.9–6.4 clean edge-to-edge; edge/centre 93–96% vs 90–91% (powerup-lesson check; the failure signature was 41%); all beats inside the 1:1 crop band; tail faded to −45.7/−31.8 vs incumbent −34.7/−25.0 (double afade 5.2s/0.8 + 5.6s/0.4). **Resolution: 720×1280 24fps — NOT the requested native 1080×1920. Generator ceiling confirmed a 4th time; §9 ask #1 stays open; the upscale pilot is the only path; preflight warning count unchanged.** **Chip zone measured at 0.0s** (the MKT-35 frame-one chip made the old 0.40s clearance obsolete): clean from frame one through 2.65s — the seated mid-shot open leaves the upper-right genuinely clear, better than the incumbent's helmet close-up. A suspected displacement glitch at 0.4s was chased to full resolution and REFUTED (misread of a packed tile — the frame is clean; the anamorphic streak is the ignition flare).

**P2 — Q1/Q2 SWEEP**: ignition beat (0–1.2s) screens are dark, nothing decorated; phone screen carries smoke only; REPORTED not judged: background monitors carry blurred chart bars and faint text-like rows, unresolvable at native resolution — same class the incumbent carries, cleared by MKT-16's eye pass. **Assembly length: 34.200s exactly** — no timeline delta.

**P3 — SWAPPED AND VERIFIED IN SITU.** Serving file = new trim; delivered 10.005s master at the convention name (old master parked `*_incumbent_backup`); candidate park + its UNREFERENCED_OK entry retired; `reel:check` 0 fail with **no new stray and no new warning class** (the public intro's two warnings are the fleet-standard 720×1280-upscale + 24fps classes). One allday_public assembled with the new intro; **contact sheet delivered with FRAME ONE first** — the thumbnail frame carries the dated chip at full opacity over the ignition open; body/stinger/endcard verified on the sheet. ⚠ ENV NOTE: this session's shell kills any ffmpeg encode at ~110s (foreground, background, and setsid all reaped; the morning detached workflow is unaffected) — the in-situ check was built as a scratchpad verification cut (ultrafast preset, same filtergraph); the local shipping file was restored from git, so the next daily run ships the new intro through the normal pipeline unchanged. §7 entry updated with the measurements. Cross-ref [[MKT-16]]/[[MKT-17]] (public intro, trim law), [[MKT-35]] (frame-one chip), MKT-06/MKT-32 (rename-corruption class).

---

### MKT-35 — Straight-match classifier + doc resync + dated chip + ribbon (2026-07-31) ✅ SHIPPED

**ID verified free** (`grep MKT-35` → audit 0 / code 0 before stamping). Content-agent work order, discovery-first with escalation gates; absorbs and withdraws the standalone dated-identity-chip order (no separate renumber needed).

**P1 — THE CLASSIFIER. The work order's mechanism was REFUTED at the gate, and the fix changed.** The order specced: hit detection compares result_digits vs bestOrder (BUG-155), so posted==drawn rows can carry hit_straight=false; fix the comparison + backfill. Queried the actual 7/30 rows first: rank 2 combo **490**/bestOrder **094** drew 094 NC midday, rank 4 combo **415**/bestOrder **415** drew 415 DC evening — **both hit_straight=TRUE in adaptive_tracking AND hitType='straight' in the snapshot**. The data layer was correct everywhere: **bestOrder is not an internal artifact, it IS the posted permutation** (the board renders `p.bestOrder ?? p.combo`; PickCard/PickDetailModal display bestOrder as the hero digits; `combo` is the engine enumeration index — the one thing never posted). BUG-155's semantics already implement posted==drawn⇒straight; the literal spec (compare vs `combo`) would have BROKEN correct semantics and was not applied. **No edge-function change. Backfill = 0 rows** — all three historical defect classes measured empty via AT⋈snapshot lateral join (A: bestOrder==actual_result ∧ ¬hit_straight; B: hit_straight ∧ bestOrder≠actual_result; C: snapshot hitType='box' ∧ AT straight). **The real defect: `render-verify-slate.ts` collapsed hitType to `Boolean` — the BOARD had no straight treatment at all**; every match rendered the same green. Fixed: straights render GOLD (#FBBF24, the verify identity) with `DREW · STRAIGHT`; box stays green. **Fail-closed asserts in `loadSlate`** (order item 5): posted==drawn must carry `hitType='straight'` and vice versa, and the board/ledger must agree on match TYPE (AT any-state-straight vs snapshot hitType) — disagreement aborts the render, extending the file's existing two-sources doctrine. **Re-render of 7/30 verified on pixels**: #2 and #4 gold, `2 OF 6 LANDED` foot intact. Ledger propagation was never broken — flags were stamped 7/30 21:14Z (NC) and 7/31 09:21Z (DC), both before the 7/31 morning assembly, so the live cut already featured both as ⭐ STRAIGHT with 2.0s holds; only the board segment misrendered.

**P2 — DOC RESYNC** (why P1's report survived a session unfixed). MASTER_AUDIT's own v2.2 close-out powerup line struck (highest-value correction — the staleness had leaked into the source of truth); stale `UNREFERENCED_OK` powerup entry removed (a registered rotation member never consults it); anchor-intros.ts's contradictory "deliberately ABSENT" comment fixed. Handoff bumped **v2.3**: MKT-31 + both addenda recorded (they were absent from the doc entirely), seal clearance + tightened binding (§3/§7/§9/v2.0-A), lockupTop-880 ruling + first-6.5s window (§3/§7), B.3 struck, §9 MKT-28/29/30 corrected, §10 MKT-16 rewritten LIVE + header date, IN-FLIGHT list, SEVEN→EIGHT reels.

**P3 — DATED IDENTITY CHIP** (operator problem: same-day saves indistinguishable at the camera-roll thumbnail — which is FRAME ONE). Chip gains line 2 `THU · JUL 31` (22px, gold #FBBF24, letter-spaced 4px) under the kind heading; **rendered at FULL OPACITY FROM FRAME ONE — fade-in removed in both assemblers** (was 0.40s onset), hold to 2.65s then the existing 0.25s fade (still clear of the crossfade at openBase−0.3). Date comes from the assembler's **provenance-asserted `stamp`** (assertBodyDate chain), never the render clock — and verify's stamp IS yesterday by construction, so the chip agrees with its own YESTERDAY'S RESULTS heading and separates the save from the same morning's All-Day drop. Chip PNG is now per-day (`_chip_<kind>_<stamp>.png`) so a stale same-kind chip can't be picked up. **Ad kinds excluded in config** (`datedChipExcluded()`: `ad`/`ad_*`) — evergreen footage must never carry a date (MKT-18 false-claim class, same ruling as MKT-21's stamp suppression); the ad lane inherits the exclusion when it exists. **Chip zone re-verified at 0.0s** (the old clearance was measured for the 0.40s onset) across all seven distinct intro files ×8 kind pairings on composited frames: ALL PASS, no member needs a nudge — `monitors` (no anchor until ~2.0s) and `public` (frame 0 is a helmet close-up; no shoulder exists in frame) are carried by the scrim plate exactly as MKT-22 designed; two-line chip bottom ~y774, inside the 420-1500 crop band. Format note: the order's literal example "THU · JUL 31" is internally inconsistent (2026-07-31 is a Friday); the FORMAT is what shipped — 7/31 renders `FRI · JUL 31`.

**P4 — RIBBON PER-SEGMENT PLACEMENT** (stopped being cosmetic once P1 made the board the legibility surface). Measured: ribbon plate y 470-590 (glow 432-651, 580px wide) crossed the #1/#2 cards once MKT-25 put the board first. Fix: the board renderer reserves a RIBBON BAND (top padding 96→328 — clear band **y 0-300 full width, held through the push-in**: content top measures 315 at max scale 1.045/origin-42%); the assembler splits the stamp overlay into two windows — **y−370 during the board segment** (plate lands y 100-220) and **y 470 from the board→summary hard cut** (a position change on a scene cut reads as staging). Ledger keeps spec placement; verified on composited max-zoom pixels: crosses no card, digit or badge. rows compress ~224→~185px, digits/badges untouched.

**COPY DELIVERED — slot 9: `OVERDUE` → `LONG GAP`** (⚠️/⚠ drop with the register, both VS16 forms, same as 🔥/❄). Collision-checked per the slot-10 lesson: OVERDUE is a substring of no other key and LONG GAP re-matches nothing; at 8 chars it is narrower than the kept STRONG SIGNAL so it fits every chip that fits. Verified through the real `lintCaption(…, 1)`: clean. DORMANT fallback not needed. Slots 7/8/10-as-shipped ACCEPTED by the content agent (recorded: keep-by-omission reasoning preferred over extra matcher entries). The MKT-28 11-row table's slot 9 is superseded by this entry.

**⚠ Takes effect next workflow run.** The LIVE 7/31 verify cut still carries the boolean-green board and y-470 ribbon — reassembly/republish of a published cut is an operator call (MKT-27/MKT-31 precedent), not taken here. Cross-ref [[MKT-31]] (identity, geometry addenda), [[MKT-25]] (board segment), [[MKT-28]] (heat table), [[MKT-22]] (chip), [[MKT-18]] (date-claim class), [[BUG-155]]/[[BUG-162]] (hit-flag semantics).
...
Handoff bumped to v2.2: the four v2.1 respec asks RETIRED; outstanding generation list = verify part-1 candidate gate (not a regen), evening_pro part 1 (unchanged). ~~anchor_intro_powerup~~ **STRUCK 2026-07-31 (MKT-35 P2): the powerup listing was STALE — the regeneration was validated and entered the rotation of 6 on 2026-07-30 (v2.0 note F, `INTRO_ROTATION` in anchor-intros.ts); it is a serving asset and owes nothing.** Queued next session: candidate gate; renumbered dated-identity-chip work order (arrived as "MKT-33" but MKT-33/34 are claimed — absorbed as [[MKT-35]] Phase 3, no separate renumber needed). Cross-ref [[MKT-32]], [[MKT-27]], [[MKT-20]].
...
**GATE — assembled 20260729 with the full package: 21.43s.** Seal stinger resolved, gold ribbon start/mid/end of body verified on frames, lattice tagged endcard resolved, endcard 100% crop clean. **Republished on explicit operator order** (2026-07-30 19:39Z — "push the repaired version live"): storage files overwritten, row upsert reset `status` to `ready` and `posted_at` to null. The prior posted stamp (10:22Z) was against the pre-MKT-31 cut, so the reset reflects reality; the assisted-handoff flow re-flips it on the next post. The MKT-27 no-republish precedent is about not wiping stamps *unasked* — this one was asked. **Superseded same evening by the geometry addendum below — the live cut is now the 25.43s one.**

---

### MKT-31 ADDENDUM — verify endcard geometry (2026-07-30, blocking postability) ✅ SHIPPED

**DIAGNOSIS REFUTED ON PIXELS — verify's endcard is NOT a legacy motion.** `endcard-config` points verify at `endcard_motion_pro.mp4`, the same file as allday_pro, and the built frames are measurably identical: bolt y 229–756 settled (249–759 at 6.2s), text glyphs 1253–1395, on BOTH kinds at both timestamps. **Delta against the 1240–1400 render: zero** — the block starts at LOCKUP_TOP 1240 (first glyph row 1253) and ends 1395, fully inside the 1150–1500 reserved band. Position was never out of contract.

**THE REAL DEFECT WAS §-SECOND-PROBLEM — the window.** The dead gap (bolt bottom 756 → text top 1253, ~497px) exists on the settled frame of EVERY motion in the set: pro 756→1253, alt 812→1253, lattice 751→1253; steady fills top (0–600) and bottom (1585–1920) but keeps the same mid-gap. So the preferred fix (motion swap) is a no-op, and the fallback (per-variant LOCKUP_TOP) is DECLINED: tucking text under a bolt that ends at ~756 means placing it ~800–1050, which breaks the reserved band — and the band exists so text is guaranteed clean background across every motion's formation frames; text at 850 would collide with the formation on every motion during its fade-in. **The acceptance's "no dead gap" + "inside the reserved band" are mutually exclusive by ~400px on every proven motion — All-Day's own accepted close ends on this exact composition.** Only a newly generated motion whose settled composition reaches the band changes that; the window fix below is its stated prerequisite.

**SHIPPED: verify's outro now takes the endcard's FIRST 6.5s, All-Day's proven window** (was: last 2.5s, a hard cut to a static stranded lockup — the thing the operator saw). Formation fills the frame, text fades in 4.5–5.2s under living motion, and the reel now ENDS at endcard t≈6.4s — smoke still wreathing the frame, bolt haloed — never reaching the barren 10s tail. Endcard audio stays unused (verify's ruling: carrier, then silence); the settled tail frame still serves the legacy fallback open; preflight bound raised 2.5→6.5s (all builds are 10.01s, margin intact).
...
**Lane regen 7/27+7/28 (2026-07-30 ~20:25Z):** both reassembled on the new pipeline and republished for lane consistency (21.13s / 28.43s); the operator countermanded mid-run but both had already completed — kept rather than spending another cycle reverting. ⚠ 7/28's REAL posted stamp (7/29 20:11Z) was wiped by its republish upsert and the storage file is no longer the cut that was posted; restore the stamp via the gateway only if the operator wants the history back.

**ADDENDUM 2 — lockup tucked under the bolt (2026-07-30 evening, operator ruling).** The operator rejected the band-compliant placement a second time; the ruling overrides the shared reserved band for verify. New per-variant `lockupTop` on `EndcardVariant` (build rig passes it through; slate kinds untouched on the shared LOCKUP_TOP 1240). **Verify = 880**, chosen from measurement, not eyeballed: the settled bolt bottoms across the pro rotation are std 760 / alt 827 (vault rings, the deepest) / lattice 751 / steady 604, and the zone y 880–1040 measures clean on all four motions from text-opaque (5.2s) to the 6.5s cut — ≤1.3% of pixels above L40, max L77 (alt's soft ring glow, fine under 700-weight white type). Block renders 880→1040 (build assert: ends y=1040, crop line 1500). The shared band's guarantee — text on clean background with NO per-motion measurement — is exactly what the override spends; it was re-purchased here with those measurements at build time, and any new motion joining verify's rotation must re-run them. Matrix + unversioned fallback rebuilt, 7/29 reassembled (25.43s unchanged) and republished (~20:45Z). **7/27 and 7/28 still carry the band placement** — their regen was countermanded as waste; regen on request only. Carry-over noted: the ribbon overlaps the first receipt row's match line mid-scroll — pre-existing stamp geometry (y≈470, pinned by spec), not introduced here; candidate for MKT-25's restaging if it bothers the operator.
...
### MKT-28 — Heat-vocabulary reconciliation + redaction over-mask ✅ FULLY CLOSED 2026-07-30 (neutral vocabulary delivered + landed)

**THE DELIVERED NEUTRAL VOCABULARY (content agent, 2026-07-30) — dropped into `HEAT_RELABEL` verbatim, replacing the provisional set. Mapped by RANK, one enum value = one string on every surface. The 11 slots:**

| # | slot | surface | current | neutral |
|---|---|---|---|---|
| 1 | temperature ≥90 | grid meter · modal banner · poster card | ON FIRE | **PEAK BAND** |
| 2 | temperature ≥80 | 〃 | BLAZING | **HIGH BAND** |
| 3 | temperature ≥65 | 〃 | HOT | **MID BAND** |
| 4 | temperature ≥45 | 〃 | WARM | **LOW BAND** |
| 5 | temperature <45 | 〃 | COOL | **BASE BAND** |
| 6 | verdict ≥80 | Heat Check | 🔥 BLAZING SIGNAL | **PEAK SIGNAL** (🔥 drops) |
| 7 | verdict ≥65 | Heat Check | ✦ STRONG SIGNAL | **keep** (already neutral) |
| 8 | verdict ≥45 | Heat Check | ◈ MODERATE | **keep** (already neutral) |
| 9 | verdict >200 draws | Heat Check | ⚠️ OVERDUE | **keep** — ⚠ FLAGGED: fits none of the four verdict rules (not a scale rung, not rising/cooling, not temperature register); kept as-is rather than guessed |
| 10 | verdict else | Heat Check | ❄ LOW | **LOW** (❄ drops — ice register). NO map entry, deliberately: WARM→'LOW BAND' and the matcher is substring-based, so a LOW entry would corrupt it |
| 11 | streak banner | grid card | 🔥 STRONG SIGNAL — Energy {n}/100 | **STRONG SIGNAL — …** (🔥 drops). NOTE: as currently rendered the leaf carries `/100`, so the Q1-absolute numeric layer HIDES it in any public frame regardless — the vocabulary entry exists for the day the denominator changes |

Rule 3 applied everywhere: numeric values survive, symbols do not — `°` dropped (caught live: the grid chip rendered `PEAK BAND 99°`), 🔥/❄ dropped not swapped. All 11 terms verified through the real `lintCaption(…, 1)`. Heat Check never enters the current capture (six pick modals only); its entries are defense-in-depth. Matcher constraint recorded at the site: longest-phrase-first, BLAZING SIGNAL before BLAZING. Cross-ref [[MKT-16]] — the vocabulary shipped in the same change as the body that verifies it.

### MKT-28 — prior close (copy decisions ruled) ✅ 2026-07-30
...
### MKT-16 — Public asset integration ✅ PHASE 1+3 SHIPPED 2026-07-30 — `allday_public` LIVE end to end

**THE HOLD CLEARED AS SPECIFIED: all five registration points landed in the same change as the validated body they verify against ([[MKT-30]] `--redact --relabel`, [[MKT-28]] vocabulary in the same change). Cross-ref [[MKT-24]] Phase 2 (captions) and [[MKT-15]] P2.**

**Migration FIRST (operator ruling, inverting the earlier last-within-the-change note): `marketing_reels_kind_check` widened to `allday_public` via MCP before any code — it is the late-failure point (render+upload succeed, upsert dies). SQL mirrored in `scripts/migrations/2026_07_30_mkt16_allday_public_kind.sql`. midday_public / evening_public stay UNREGISTERED — no session carriers exist and three daily public reels is the cadence risk already ruled against.**

**The five points, each verified:** carrier `public_carrier.mp4` + `_pt2` explicit pair (joined 20.360s, margin +0.331s — identical to allday_free's, the tier invariant confirming itself); endcard free-tier motion set, `HITMASTER ZK6 / THE FULL BOARD IS FREE / JOIN THE COMMUNITY` (matrix built ×2 free motions, copy frame eye-verified sans-serif inside the crop); stinger shared set, `HITMASTER ZK6 / THE FULL BOARD` (×5 motions built) — the same string as the intro chip, deliberately; scope registry `public` variant with `publicVariants` stated explicitly and a three-mode `captureModeFor` (full/redacted/public); captions = the EIGHT DELIVERED MKT-24 templates, **recovered verbatim from the delivery transcript** (they were measured and lint-cleared 7/29 but never committed — lengths match the recorded 180/163/148/143/192/160/141/132 byte-for-byte, re-linted clean at tier 1), offset 4 = the one free residue mod 8. Plus: publish `Kind` union, `ReelKind`/`KIND_UI` (defaultTarget **cross** — the Two-Question-gated class), a `'public'` caption audience the tier-4 gate refuses with its own reason, `assertBodyPublic` (PUBLIC_TAG required, no lenient direction), `--variant` filter added to the ASSEMBLER (the publish-side flag one step earlier), and the public stamp drops the scope tag (`THU · JUL 30`, verify's scope-less form — the stamp is composited at assembly, the one layer capture audits cannot see).

**THE FIRST RENDER ABORTED FIVE WAYS, EVERY ONE A REAL CLASS — this is why registration waited for a real body:** (1) `+169` hit-dot overflow counts (six leaves) → third lexical shape carve-out `+N` in `assertNoDigits`, evidence-based per the file's own rule; all 9 combination arrangements still flag. (2) The pre-capture audit ran against the LIST view — which renders SignalBar BOX/CO labels, the `Box:` line, LastHitPill and the band footer, none of which reach a captured frame — audit moved after the Grid toggle, still before frame 0. (3) Bare state-code leaves (`TX`, `FL`) and the Tri-State `ME,NH,VT` → attribution-shaped-leaf suppression. (4) `straight` in a text node appended to a childless leaf → swaps extracted to `swapStr`, applied to mixed/appended text nodes. (5) **THE ROOT CAUSE UNDER #4, and it lived in the REDACTION TOO: `sweep(root)`/`maskAll(root)` used `querySelectorAll`, which returns DESCENDANTS ONLY — an individually-appended leaf element was swept as a root with no descendants and processed as NOTHING.** Fixed in both files; in reel-redact an appended text node into a childless parent also fell through both observer branches. That was a live hole in the free-session lane's masking, mount-order dependent.

**THE EYE PASS THEN CAUGHT TWO THE ENGINE AUDITS STRUCTURALLY COULD NOT:** `PEAK BAND 99°` (rule 3 — degree symbol drops; added to HEAT_RELABEL) and an **`OK 1×` Oklahoma attribution that PASSED the audit** because brandLint's STATE_CODES deliberately omits English-word collisions (IN/OK/OR/ME/OH/HI — widening it flagged `RESOLVED IN · LAST 30 DAYS`). Resolution: the lint set stays partial for prose, with the reason now documented at the site; the CAPTURE gets `STATE_CODES_COMPLETE` (50+DC) applied by BOTH the suppression and the audit to attribution-SHAPED strings only (every token code-shaped) — strictly harder than the caption lint, blind to English words. **A scope passing this guard proves nothing about the next day's content — third confirmation of that MKT-26 lesson (+169, OK, ME/NH/VT were all day-dependent).**

**SHIPPED:** `allday_public_20260730.mp4` 34.20s (6.0s public intro + 2.7s stinger + 19.0s public body + 6.5s endcard), overlap mode with 0.96s boundary slack (OVERLAP_EPSILON non-interaction at the measured body length), published `--variant=public` — row `ready`, posted pro/free siblings untouched. Grid + all six modals audited 0 blocking violations at capture; contact sheet + full-res grid/modal frames eye-verified. `reel:daily` now builds EIGHT reels (reel:allday renders the public body third).

**THE STORED Q1/Q2 RECORD** (`assets/marketing/_public_gate_records.json`, required before the kind publishes — **`reel:check` FAILs a public kind without it**, verified live before the record existed): who looked, at what density, all five classes; fabricated-numerals and rendered-dates recorded as CODE PROPERTIES for this rendered body; the scope segment tabs recorded as visible (advisory class per §5 — session words are hard-forbidden only beside digits, and every digit is masked).

**PLATFORMS: YouTube, TikTok, Reddit and X now have their `asset:'redacted'` gate condition MET and are UNBLOCKABLE — none flipped.** Enabling is operator-only, one platform at a time, Two-Question checkboxes on every handoff. Instagram remains do-not-build (Meta cross-enforcement). The cross-post target inherits eligibility automatically — same requirement, now satisfiable per reel via the stored record + Two-Question ack.

### MKT-16 — original scope 🔍 PHASE 0 REPORTED · BUILD HELD (superseded above)
