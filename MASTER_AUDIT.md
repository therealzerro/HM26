...
- **One asset, every reel kind.** `assets/marketing/anchor_intro.mp4` — the Anchor (visored newsroom figure) raises a phone, the camera dives into the screen, resolving to full-frame purple smoke. It REPLACES the legacy 1.2s endcard-lockup open in both `assemble-allday-reels.ts` and `assemble-verification-reel.ts`; the endcard remains the outro, untouched. All-Day 23.7s → **29.0s**, verify 10.0s → **15.3s**.
...
- **Delivered asset needed a trim — recorded because it is a recurring asset-spec lesson.** The 2026-07-27 delivery arrived at **10.0s** (contract 3.5–6.5s) and preflight correctly failed it. Worse than the length: its last 2.5s **pull the camera back OUT to the newsroom**, so the file did not end on the smoke bed the dissolve consumes. Frame-stepping put the last clean full-smoke frame at t≈7.5s (newsroom bleed-through from 7.6s). Trimmed **1.0→7.5s = 6.5s** — exactly the contract ceiling, no code change — keeping ~0.5s of settled-Anchor recognition before the phone appears. Delivered master preserved as `anchor_intro_master_10s.mp4` (not read by the pipeline) so the trim stays reproducible.
- **Phase 2 verification, real asset, all three reels assembled on throwaway stamp `19700101` (live 7/27 reels untouched, samples deleted after review):** durations exact (29.0 / 29.0 / 15.3); loudness pro −15.74, free −14.56, verify −14.43 LUFS integrated — pro is in family with the live legacy reels (7/27 and 7/26 both −15.55), i.e. not an intro regression; **no click risk at either new join** — the intro's sting rises from and decays back into true digital silence (−101 dB) before the dissolve, confirmed by per-0.5s RMS; **1:1 centre-crop check (the risk flagged at Phase 0) CLEARS** — `crop=1080:1080:0:420` keeps the visor, the phone-raise gesture and the smoke, clipping only the top of the helmet. Review sheets kept in `assets/marketing/mkt08_gate/`.
- **Two open items, both for the operator, neither blocking:** (1) reel length grows +22% (23.7→29.0s); (2) the reel now opens with ~1.0s of silence — the intro's sting starts 1.0s into the master and the VO doesn't enter until 6.1s, where legacy had VO from t=0. Cutting the intro from 1.9s instead (5.6s, audio immediate) fixes it but cuts in mid-gesture and costs the Anchor reveal. Also surfaced by the tightened ceiling: the **free** carrier's wall-to-wall VO still runs to ~17.5s and needs a trim to ~17.0s on its next revision.
- Docs updated: handoff `Reel_System_Handoff_2026-07-27.txt` → v1.3 (§1 durations, §2 timeline + carrier-local VO beat map, §3 verify timeline, §7 inventory rows + **Anchor intro delivery spec**), `REEL_COMMANDS.txt` durations.
