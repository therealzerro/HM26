...
export default function ImportWizardScreen() {
  ...
  const [dataExistsWarning, setDataExistsWarning] = useState<string | null>(null);
  const [hitDetectionStatus, setHitDetectionStatus] = useState<'idle' | 'running' | 'done'>('idle');
  const [hitDetectionResult, setHitDetectionResult] = useState<HitDetectionResult | null>(null);
  ...
  const parseDateLoose = (s: string): string => {
    ...
    if (!trimmed) throw new Error('Invalid date format: empty string');
    ...
    try {
      ...
      const alphaMatch = trimmed.match(/(?:[A-Za-z]{3,},?\s+)?([A-Za-z]{3,})\s+(\d{1,2}),?\s*(\d{4})/i);
      if (alphaMatch) {
    ...
    throw new Error(`Invalid date format: "${trimmed}". Expected YYYY-MM-DD or Mon DD, YYYY`);
  };
  ...
  const checkDataExists = async (): Promise<boolean> => {
    if (importType !== 'box_history' && importType !== 'pair_history') return false;
    try {
      const table = importType === 'pair_history' ? 'datasets_pair' : 'datasets_box';
      const classFilter = importType === 'pair_history' ? `&class_id=eq.${classId}` : `&class_id=eq.1`;
      const path = `/rest/v1/${table}?select=id&scope=eq.${encodeURIComponent(scope)}${classFilter}&horizon_label=eq.${encodeURIComponent(horizonLabel)}&limit=1`;
      const rows = await fetchFromSupabase<any[]>({ path });
  ...
  const handleCommit = async () => {
    if (validationErrors.length === 0 && csvData.trim()) {
      try {
        ...
        setDataExistsWarning(exists ? `⚠️ Data already exists for ${scope} ${horizonLabel}. Importing will UPDATE existing rows with new values.` : null);
      ...
      <ScrollView style={styles.content}>
        ...
        {dataExistsWarning && (
          <View style={styles.existsWarning}>
            <Text style={styles.existsWarningText}>{dataExistsWarning}</Text>
          </View>
        )}
