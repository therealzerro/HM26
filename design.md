...
| 6 | Drop `D` alias map in Results; hardcoded radii → tokens | 30 min | ✅ shipped | Refactor only; no visual change. Future contributors must use the design system. |
...
- 2026-05-13 — Step 6 — Local `D` color alias map in `results.tsx` retired; all 50 `D.teal/D.amber/D.surface/...` references replaced with `theme.colors.cyan/gold/bgElevated/...` directly. Hardcoded `borderRadius` literals (`12`, `14`, `16`, `22`) across Home/Slate/Results replaced with `theme.borderRadius.{tile,lg,card,xxl}` tokens. Two `borderRadius: 13` literals on Home buttons left as-is (no token equivalent). Zero visual change — pure refactor. Future contributors must reach for the design system instead of accidentally introducing a fourth value variant.
- 2026-05-13 — Slate fix — included `'top'` in SafeAreaView edges. The retired `statusStrip` (step 5) had been the de-facto top buffer below the notch; without it the gradient header crept into the unreachable zone. Slate now matches Home and Results in top-inset handling.
