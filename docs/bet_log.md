...
**Positions — PROPOSED (late-29 pool, $7.25/unit; units are the operator's — shape matches the 9/13 early-pool ticket)**
...
**Total: $36.25** (1S + 1B = $14.50). Straight-only on 681 (a 186 / 861 / 168 draw pays nothing on this ticket); box-only on {4,6,9} (any order pays $112.50 per hitting state). Session math on 29 draws (uniform): ~18.4% that the ticket pays anything, ~2.9% the straight, ~16.0% the box; expected ≈ −$3.60 at 90% RTP. If the all-day 4-leg ticket was placed this morning, both sets are already covered on these 29 as part of the 75 — this stacks on top (a 681 draw pays both tickets; a {4,6,9} draw pays the box here plus the 469 box leg, and 469 straight there). Reconcile against the late-29 after the 9/15 import (operator paste welcome; last draw NM 11:15pm ET).
