...
**Content:** a single line: "7-day hit rate: 73% · last 7 days: 5 HITS · today's slate avg energy: {avgEnergy}". Numbers come from the existing `daily_intelligence` table — same query AdaptiveLearningView uses.
**Why:** the engine's whole reason to exist is the hit rate. Subscribers should see it as the second thing on screen (after the avg energy hero). Effort: **2 hours**. Impact: trust + retention.
