...
const UNREFERENCED_OK: Record<string, string> = {
  ...
  'anchor_intro_arrive.mp4':
    'MKT-54 GATED-OUT 2026-08-13 — trimmed (IN 0.310/OUT 6.510, clean smoke bed, chip/band/audio all pass) but its desk-front display carries a legible white "S" letterform from FRAME ONE (the camera-roll thumbnail) to ~1.2s, breaching the no-glyph wall bar board was audited against for this rotation. Operator ruling owed: (a) accept as set dressing and register (one line in INTRO_ROTATION), (b) re-trim IN≈1.35/OUT≈7.55 — clears the glyph, pull-back margin ~0.35s, but DELETES the empty-desk beat that is the concept, or (c) regenerate without the desk sign. Master preserved as anchor_intro_arrive_master_10s.mp4.',
