...
async function main() {
  ...
  console.log('\n═══════════════════════════════════════════════════════════════');
  console.log('  SIGNAL-INFO-01: stratified full-universe AUC (read-only)');
  console.log(`  Window: ${from} → ${to} (${days.length} days), scopes: ${scopes.join(', ')}`);
  console.log('  Fair-draw null hypothesis: singles-stratum AUC = 0.500');
  console.log('═══════════════════════════════════════════════════════════════');
  ...
  for (const scope of scopes) {
    ...
    for (const day of days) {
      ...
      // Score every comboset.
      const scored: { mult: string; hit: boolean; BOX: number; PBURST: number; CO: number; DGC: number }[] = [];
