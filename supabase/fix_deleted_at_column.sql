-- Fix deleted_at column issue for slate_snapshots table
-- This script ensures the deleted_at column exists and all related functionality works
...
-- Step 3: Update the view to include deleted_at and filter out deleted records
...
-- Step 6: Verify data exists and insert comprehensive test data if needed
DO $$
DECLARE
...
BEGIN
  ...
  IF snapshot_count = 0 THEN
    ...
    -- Insert comprehensive test data for all scopes
    ...
    (
      'midday', 
      'test_hash_midday_' || extract(epoch from now())::text, 
      'test_hash_midday_' || extract(epoch from now())::text,
    ...
    (
      'evening', 
      'test_hash_evening_' || extract(epoch from now())::text, 
      'test_hash_evening_' || extract(epoch from now())::text,
    ...
    (
      'allday', 
      'test_hash_allday_' || extract(epoch from now())::text, 
      'test_hash_allday_' || extract(epoch from now())::text,
...
DO $
BEGIN
  RAISE NOTICE 'SCHEMA FIX COMPLETE: deleted_at column issue resolved';
END $;